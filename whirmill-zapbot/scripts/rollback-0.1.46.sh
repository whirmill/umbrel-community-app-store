#!/bin/sh
# Run only after reviewing a 0.1.63 rollback. This is a compatibility rollback:
...
# 0.1.63 package graph to have completed first; it never initializes credentials
...
current_image='ghcr.io/whirmill/zapbot:umbrel-execution-window-30470c8e300b301b2ae2dd29cffd227b549d27fc@sha256:2a1b41c1c8b9b05a0e267a67fb51a6a7a2b1502e50c6dada4400b7acf6d18227'
# current_image is synchronized with every 0.1.63 Compose image identity.
...
verify_images() {
  ...
  for service in release-sql-export migrate; do
    ...
    test "$image" = "$current_image" || { echo "unexpected 0.1.63 release image for $service" >&2; exit 67; }
  done
}
...
classify_runtime() {
  ...
  for service in release-sql-export migrate normalize-and-verify; do
    service_id=$(compose ps -aq "$service" | tail -n 1)
    test -n "$service_id" || { echo "missing completed 0.1.63 bootstrap service: $service" >&2; exit 67; }
    test "$(docker inspect -f '{{.State.Status}}:{{.State.ExitCode}}' "$service_id")" = 'exited:0' || {
      echo "rollback requires completed 0.1.63 bootstrap service: $service" >&2
      exit 67
    }
  done
  ...
  for service in release-sql-export migrate; do
    ...
    test "$(docker inspect -f '{{.Config.Image}}' "$service_id")" = "$current_image" || {
      echo "rollback requires current 0.1.63 release image for $service" >&2
      exit 67
    }
  done
}
