#!/bin/sh
# Run only after reviewing a 0.1.64 rollback. This is a compatibility rollback:
...
# 0.1.64 package graph to have completed first; it never initializes credentials
...
current_image='ghcr.io/whirmill/zapbot:umbrel-h4-diagnostics-6e636710ffd2cf9d0c38f8786e07fe0d1ca5ab6d@sha256:9906c9e8810a1635ee10f87c48a1edac96835b036dc5781796a2fd2142671871'
# current_image is synchronized with every 0.1.64 Compose image identity.
...
verify_images() {
  ...
  for service in release-sql-export migrate; do
    ...
    test "$image" = "$current_image" || { echo "unexpected 0.1.64 release image for $service" >&2; exit 67; }
  done
}
...
classify_runtime() {
  ...
  for service in release-sql-export migrate normalize-and-verify; do
    service_id=$(compose ps -aq "$service" | tail -n 1)
    test -n "$service_id" || { echo "missing completed 0.1.64 bootstrap service: $service" >&2; exit 67; }
    test "$(docker inspect -f '{{.State.Status}}:{{.State.ExitCode}}' "$service_id")" = 'exited:0' || {
      echo "rollback requires completed 0.1.64 bootstrap service: $service" >&2
      exit 67
    }
  done
  ...
  for service in release-sql-export migrate; do
    ...
    test "$(docker inspect -f '{{.Config.Image}}' "$service_id")" = "$current_image" || {
      echo "rollback requires current 0.1.64 release image for $service" >&2
      exit 67
    }
  done
}
